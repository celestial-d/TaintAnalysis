...
#define PREDEFINE "DEF"
#define ACPI "ACPI"
#define RWPAGE "RWPAGE"
#define DAX "DAX"
#define NDOPEN "NDOPEN"
#define GETGEO "GETGEO" 
#define MOUNT "MOUNT" 
    ...
    private:
        ...
        void setupFunctionArgs(Function *targetFunction, GlobalState &targetState, std::vector<Instruction *> *callSites) {
            ...
            if (functionType == MOUNT) {
                taintedArgData.insert(0);
                is_handled = true;
            }
            if (functionType == GETGEO) {
                taintedArgs.insert(0);
                is_handled = true;
            }
            if (functionType == NDOPEN) {
            ...
            if (functionType == RWPAGE) {
                pointerArgs.insert(0);
                taintedArgs.insert(1);
                taintedArgData.insert(2);
                taintedArgs.insert(3);
                is_handled = true;
            }
            if (functionType == ACPI) {
                pointerArgs.insert(0);
                is_handled = true;
            }
            if (functionType == PREDEFINE) {
                pointerArgs.insert(0);
                is_handled = true;
            }
